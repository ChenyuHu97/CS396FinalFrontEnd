...
function searchItem(){
	let result = "";
	let searchName = document.getElementById("searchGame").value;
	if(searchName.length != 0){
		for(let i =0; i<gameList.length;i++){
			let lowerGame = searchName.toLowerCase();
			let gameName = gameList[i].gameName.toLowerCase();
	...
	if(result.length == 0){
		Swal.fire({
            icon: 'error',
            title: 'Failed',
            text: "No result with this search term"
        })
	}
