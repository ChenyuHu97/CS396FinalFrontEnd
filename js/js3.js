...
function listSearch(){
    let searchTerm =  document.getElementById("listSearch").value;
    let result ="";
    ...
    if(searchTerm.length != 0){
        ...
        for(let i =0; i<myList.length;i++){
			let lowerGame = searchTerm.toLowerCase();
			let gameName = myList[i].gameName.toLowerCase();
            console.log(lowerGame);
            console.log(gameName);
            if(gameName.indexOf(lowerGame) != -1){
                result += `
                <tr>
    ...
    console.log(result);
    document.getElementById("videogameList").innerHTML =  result;
}
